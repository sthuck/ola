...
def main():
    ...
    min_expectation_graph = min_expectation_and_replacement_algo(g.copy(), True)
    ...
    for i in range(iterations):
        ...
        results.append((len(g.es), naive_ola_distance, min_expectation_ola_distance))


    # df = pd.DataFrame(results, columns=["edges", "naive", "algo2", "min-exp"])
    df = pd.DataFrame(results, columns=["edges", "naive", "min-exp"])
